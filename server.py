...
kitDir = os.path.dirname(os.path.realpath(__file__)) + '/kits/'
...
class Game:
    """
    Contains current game state as jsonized svg, and clients in the game.
    """
    expiresIn = 60 # sec

    def __init__(self, type, name):
        boardFilePath = kitDir + type + '/board.json'
        self.state = GameState(boardFilePath)
        self.name = name
        self.id = self._generateId()
        self.clients = []
        self.timeAbandoned = None
        
        self.displayName = 'Game display name not set'
        for gameTypeDict in gameTypes:
            if gameTypeDict[u'gameType'] == type:
                self.displayName = gameTypeDict[u'displayName']
        
    def _generateId(self):
        """Return random 30 char string matching [A-Za-z0-9]"""
        size = 30
    ...
    def getPlayersUpdate(self):
        ...
        msgDict = {'messageType': 'playersUpdate', 
                   'players': {c.peerstr : c.name for c in self.getClients()}}
        return Envelope(msgDict, self.getClients())
        
    def isExpired(self):
        """Return boolean; true only if empty, abandoned, and old."""
        if len(self.clients) == 0 and not self.timeAbandoned is None:
...
class GameState:
    ...
    Information is divided into two sets:
        boardDict: svg-encodable info: path, position, rotation of game pieces
        metaDict: non-svg-encodable info: moveable, stackable, shuffleable
    """
    
    def __init__(self, boardFilePath):
        boardFile = open(boardFilePath, 'r')
        self._objList = json.loads(boardFile.read())
    
    def _fixJson(self, jsonStr):
        """
        NOT CURRENTLY USED, kept around because the regex's are useful.
        Correct unfortunate errors created in the parsing of svg to json
        via svg2gfx.xslt.
        """
        patterns = {
            (r',(\]|\})', r'\1'),                       # remove extra commas
            (r'\}\{', '},{'),                           # add missing commas
            (r'([{\[,])\s*([a-zA-Z0-9_,]+?):', r'\1"\2":') # quote keys
        }
        for search, replace in patterns:
            jsonStr = re.sub(search, replace, jsonStr)
        return jsonStr
        
    def get(self):
        return self._objList
        
    def set(self, svgobjId, prop, valDict):
        targetDict = None
        for svgDict in self._objList:
            if svgDict[u'id'] == svgobjId:
                targetDict = svgDict
                break
        if targetDict is None:
            ...
            raise Exception('Cant set state, id not found: ' + svgobjId)
        targetDict[prop] = valDict
...
class MessageHandler:
    ...
    def svgobjSetProp(self, sender, msgDict):
        """
        Record changed object property with correct game state and
        broadcast it to other players in the game so they can update
        their views/models.
        """
        game = gameRegistry.get( msgDict[u'gameId'] )
        game.state.set(msgDict[u'svgobjId'],msgDict[u'property'],
                       msgDict[u'value'])
        
        # don't send things to back to the sender
        clients = [c for c in game.getClients() if c.peerstr != sender.peerstr]
        setProp = Envelope(msgDict, clients)
        
        return (setProp,)

    def initGame(self, sender, msgDict):
        """
        ...
        if not gameRegistry.gameExists(msgDict[u'gameId']):
            ...
            badId = Envelope({'messageType': 'error', 'message': 'bad url'},
                             [sender])
            return (badId,)
    ...
    def requestNewGame(self, sender, msgDict):
        """Make a new game. This message type issues from the lobby."""
        game = Game(msgDict[u'gameType'], msgDict[u'gameName'])
        print msgDict[u'gameType']
...
class ServerProtocol(WebSocketServerProtocol):
    ...
    def onMessage(self, msgStr, binary):
        if binary: raise Exception('handling binary messages not written')
...
class ServerFactory(WebSocketServerFactory):
    ...
    Got the code from an example on http://autobahn.ws/
    """
...
if __name__ == '__main__':
   ...
   gameTypes = json.loads(open('game_types.json', 'r').read())
